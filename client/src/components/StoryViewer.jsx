  ...
  return (
    ...
    >
      ...
      <AnimatePresence mode="wait">
        <MotionDiv
          key={active._id}
          className="story-cinema-card w-full max-w-md overflow-hidden rounded-3xl bg-black"
          initial={{ scale: 0.45, opacity: 0, rotateX: 18, filter: "blur(18px)" }}
          animate={{ scale: 1, opacity: 1, rotateX: 0, filter: "blur(0px)" }}
          exit={{ scale: 1.18, opacity: 0, filter: "blur(22px)" }}
        ...
        >
          {active.mediaType === "video" ? (
            ...
            <img
              src={active.mediaUrl}
              alt="Story"
              className="ken-burns-media max-h-[80vh] w-full object-cover"
              onError={(e) => {
                e.target.style.display = "none";
                const fallback = document.createElement("div");
                fallback.className = "flex flex-col items-center justify-center h-64 gap-2";
                fallback.innerHTML = '<span style="font-size:3rem">Image</span><p style="color:#9ca3af;font-size:0.875rem">Image no longer available</p>';
